...
public class RgbColorPickerWidget extends AbstractWidget {
	...
	public RgbColorPickerWidget(int x, int y, int contentWidth, int initialColor, IntConsumer onColorChanged, double scale) {
		...
		svTexture = new DynamicTexture(() -> "hologrammenu_sv_picker", svImage);
		hueTexture = new DynamicTexture(() -> "hologrammenu_hue_picker", hueImage);

		int seq = TEXTURE_SEQ.incrementAndGet();
		svTextureId = HologramMenuMod.id("dynamic/color_sv_" + seq);
		hueTextureId = HologramMenuMod.id("dynamic/color_hue_" + seq);
