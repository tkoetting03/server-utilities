...
public class HologramOptionsScreen extends Screen {
	...
	protected void init() {
		...
		for (int index = 0; index < visibleLineCount; index++) {
			...
			addRenderableOnly(new LineBackingWidget(
				...
				contentWidth,
				boxHeight,
				Component.translatable("screen.hologrammenu.hologram_options.line_label", lineIndex + 1)
			));
	...
	private EditBox createLineField(int fieldX, int currentY, int contentWidth, int rowHeight, int index, HologramLineStack.Line line) {
		EditBox field = new EditBox(font, fieldX, currentY, contentWidth - LINE_ACTION_AREA_WIDTH, rowHeight, Component.translatable("screen.hologrammenu.hologram_options.text"));
	...
	private Component seeThroughButtonLabel() {
		return Component.translatable(seeThroughWalls
			? "screen.hologrammenu.hologram_options.see_through_on"
			: "screen.hologrammenu.hologram_options.see_through_off");
	}
	...
	private int addPageNavigation(int fieldX, int currentY, int contentWidth, int rowHeight, int rowGap) {
		...
		addRenderableOnly(new PageStatusWidget(
			...
			statusWidth,
			rowHeight,
			Component.translatable("screen.hologrammenu.hologram_options.page_status", page + 1, maxPage + 1)
		));
