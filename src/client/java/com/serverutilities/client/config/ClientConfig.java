...
public final class ClientConfig {
	...
	private static final Path CONFIG_PATH = FabricLoader.getInstance()
		.getConfigDir()
		.resolve(ServerUtilitiesMod.MOD_ID + "-client.json");
	...
	public static void load() {
		...
		try (Reader reader = Files.newBufferedReader(CONFIG_PATH)) {
			...
			ServerUtilitiesMod.LOGGER.error("Failed to load client config, using defaults", exception);
			instance = new ClientConfig();
		}
	...
	public static void save() {
		try {
			...
			ServerUtilitiesMod.LOGGER.error("Failed to save client config", exception);
		}
	}
