...
public final class HologramAssociationHighlighter {
	...
	private static void render(LevelRenderContext context) {
		...
		if (ClientSettings.placementModeEnabled || ClientSettings.hologramEditModeEnabled) {
			for (Entity entity : level.entitiesForRendering()) {
				...
				Optional<ItemEntity> item = associatedItem(level, display);
				if (item.isPresent()) {
				...
				Optional<BlockPos> block = HologramClientRegistry.associatedBlock(display)
					.or(() -> HologramHelper.associatedBlock(display));
	...
	private static Optional<ItemEntity> associatedItem(ClientLevel level, Display.TextDisplay display) {
		Optional<UUID> uuid = HologramClientRegistry.associatedEntityUuid(display)
			.or(() -> HologramHelper.associatedEntityUuid(display));
		if (uuid.isEmpty()) {
			return Optional.empty();
		}
		for (Entity entity : level.entitiesForRendering()) {
