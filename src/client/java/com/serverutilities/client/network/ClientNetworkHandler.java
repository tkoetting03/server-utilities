package com.serverutilities.client.network;

import com.serverutilities.client.hologram.HologramClientRegistry;
import com.serverutilities.client.npc.NpcClientConfigStore;
import com.serverutilities.client.npc.NpcClientRegistry;
...
import com.serverutilities.client.screen.HologramOptionsScreen;
import com.serverutilities.client.screen.StorageMenuEditorOverlay;
import com.serverutilities.client.storage.ShopClientState;
import com.serverutilities.client.storage.StorageMenuAssociatedBlocks;
...
public final class ClientNetworkHandler {
	...
	public static void register() {
		...
		ClientPlayNetworking.registerGlobalReceiver(ModPackets.ShopStatePayload.TYPE, (payload, context) -> {
			Minecraft.getInstance().execute(() -> {
				java.util.Map<Integer, com.serverutilities.storage.ShopListing> listings = new java.util.HashMap<>();
				for (com.serverutilities.storage.StorageMenuNetwork.ShopListingData listing : payload.listings()) {
					com.serverutilities.storage.ShopListing resolved = listing.toListing();
					if (resolved.isConfigured()) {
						listings.put(resolved.slotIndex(), resolved);
					}
				}
				ShopClientState.set(payload.pos(), new com.serverutilities.storage.ShopDefinition(payload.shopEnabled(), listings));
			});
		});
