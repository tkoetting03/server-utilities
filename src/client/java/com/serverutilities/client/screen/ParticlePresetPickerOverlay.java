...
import com.serverutilities.client.screen.widget.ParticlePresetGridWidget;
import com.serverutilities.client.screen.widget.UiScale;
import com.serverutilities.particle.ParticlePresetCatalog;
import com.serverutilities.particle.ParticlePresetCategories;
...
public final class ParticlePresetPickerOverlay {
	...
	private void buildWidgets() {
		...
		var panelWidget = new PresetPickerPanelWidget(
			...
			panelWidth,
			panelHeight,
			Component.translatable("screen.serverutilities.particle_presets.title")
		);
		...
		attachLabel(font, contentLeft, searchLabelY, contentWidth, Component.translatable("screen.serverutilities.particle_presets.search"));
		int searchFieldY = searchLabelY + LabeledFieldLayout.LABEL_HEIGHT + LabeledFieldLayout.LABEL_GAP;
		searchField = new EditBox(font, contentLeft, searchFieldY, contentWidth, fieldHeight, Component.translatable("screen.serverutilities.particle_presets.search"));
		...
		screenInvoker.serverutilities$addRenderableOnly(statusLabel);
	...
	private Component categoryLabel() {
		return Component.translatable("screen.serverutilities.particle_presets.category", formatCategory(category));
	}
	...
	private void updateStatusLabel() {
		...
		Component message = Component.translatable(
			"screen.serverutilities.particle_presets.status",
			totalCount,
			page + 1,
			maxPage
		);
	...
	private void attachLabel(Font font, int x, int y, int width, Component text) {
		...
		screenInvoker.serverutilities$addRenderableOnly(label);
		widgets.add(label);
		dragGroup.track(label);
	}

	private void attachInteractive(AbstractWidget widget) {
		screenInvoker.serverutilities$addRenderableWidget(widget);
