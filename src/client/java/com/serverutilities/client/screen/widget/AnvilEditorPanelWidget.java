...
public class AnvilEditorPanelWidget extends AbstractWidget {
	...
	private static Component panelTitle(AnvilEditorTab activeTab) {
		return switch (activeTab) {
			case STYLE -> Component.translatable("screen.serverutilities.text_style.title");
			case LORE -> Component.translatable("screen.serverutilities.anvil.lore_title");
		};
	}
		...
		UiScaleText.withScale(graphics, x, y, () -> {
			...
			if (activeTab == AnvilEditorTab.LORE) {
				UiScaleText.draw(
					graphics,
					font,
					Component.translatable("screen.serverutilities.anvil.lore_lines_label"),
					contentLeft,
