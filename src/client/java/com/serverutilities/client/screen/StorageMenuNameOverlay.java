...
import com.serverutilities.storage.StorageMenuItemNames;
	...
	private void buildWidgets() {
		...
		if (!styleEditing) {
			nameField = new EditBox(
				...
				Component.translatable("screen.serverutilities.storage_menu.name_field")
			);
			nameField.setMaxLength(64);
			nameField.setValue(TextFormats.parse(styledName).text());
			nameField.setHint(Component.translatable("screen.serverutilities.storage_menu.name_hint"));
		...
		attachButton(Button.builder(
			Component.translatable("screen.serverutilities.hologram_options.style"),
			press -> openStylePanel()
		).bounds(fieldX, buttonRowY, buttonWidth, buttonH).build());

		attachButton(Button.builder(
			Component.translatable("screen.serverutilities.storage_menu.name_clear"),
			press -> clearName()
		...
		DraggableTitleBarWidget titleBar = dragGroup.createTitleBar(
			Component.translatable("screen.serverutilities.storage_menu.name_title"),
			StorageMenuNamePanelWidget.PANEL_WIDTH,
			UiScale.s(14)
		);
