...
import com.serverutilities.client.screen.widget.NpcHologramStackPanelWidget;
import com.serverutilities.client.screen.widget.NpcStackDragHandle;
...
public final class NpcHologramStackOverlay implements NpcStackDragHandle.Listener {
	...
	private void buildWidgets(int panelX, int panelY) {
		...
		DraggableTitleBarWidget titleBar = dragGroup.createTitleBar(
			Component.translatable("screen.serverutilities.npc_options.holograms_title"),
			panelWidth,
	...
	private void buildEntryRows(int panelX, int panelY, int partCount, TextStylePanelLayout.Metrics layout, int buttonHeight) {
		...
		for (int index = 0; index < partCount; index++) {
			...
			if (isName) {
				EditBox field = new PartSelectEditBox(
					...
					Component.translatable("screen.serverutilities.npc_options.stack_name"),
				...
				EditBox field = new PartSelectEditBox(
					...
					holoFieldWidth,
					LabeledFieldLayout.FIELD_HEIGHT,
					Component.translatable("screen.serverutilities.npc_options.stack_hologram", hologramNumber(index)),
					() -> selectedIndex = rowIndex
				);
				...
				attach(Button.builder(Component.translatable("screen.serverutilities.hologram_options.style"), press -> openLineStyle(capturedIndex))
		...
		if (partCount < NpcHologramStack.MAX_ENTRIES) {
			...
			attach(Button.builder(Component.translatable("screen.serverutilities.npc_options.add_hologram"), press -> addHologram())
	...
	private void buildFooter(int panelX, int panelY, int partCount, TextStylePanelLayout.Metrics layout, int buttonHeight) {
		...
		attach(Button.builder(Component.translatable("screen.serverutilities.npc_options.add_above"), press -> addRelative(-1))
			.bounds(left, footerY, contentWidth, buttonHeight).build(), true);
		attach(Button.builder(Component.translatable("screen.serverutilities.npc_options.add_below"), press -> addRelative(1))
