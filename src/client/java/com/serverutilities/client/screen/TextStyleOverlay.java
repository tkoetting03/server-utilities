...
import com.serverutilities.client.mixin.accessor.MultiLineEditBoxAccessor;
import com.serverutilities.client.mixin.accessor.MultilineTextFieldAccessor;
...
import com.serverutilities.client.screen.widget.ClassicColorSwatchButton;
import com.serverutilities.client.screen.widget.DraggableTitleBarWidget;
import com.serverutilities.client.screen.widget.GradientPreviewWidget;
...
import com.serverutilities.client.screen.widget.PartSelectEditBox;
import com.serverutilities.client.screen.widget.RgbColorPickerWidget;
import com.serverutilities.client.screen.widget.TextStylePanelLayout;
...
import com.serverutilities.client.screen.widget.AnvilEditorMetrics;
...
import com.serverutilities.text.StyledSpan;
...
public final class TextStyleOverlay {
	...
	private void buildWidgets(int panelX, int panelY) {
		...
		if (isAnvilMode()) {
			...
			screenInvoker.serverutilities$addRenderableOnly(panelWidget);
		...
		DraggableTitleBarWidget titleBar = dragGroup.createTitleBar(
			isAnvilMode()
				? Component.translatable("screen.serverutilities.anvil.editor_title")
	...
	private void buildStyleWidgets(int panelX, int panelY, int contentTopOffset, int partCount) {
		...
		if (gradientExpanded) {
			...
			gradientStartButton = Button.builder(Component.translatable("screen.serverutilities.text_style.gradient_start"), press -> {
				...
				.build();
			gradientEndButton = Button.builder(Component.translatable("screen.serverutilities.text_style.gradient_end"), press -> {
	...
	private Runnable[] footerHandlersFor(Component[] labels) {
		...
		for (int index = 0; index < labels.length; index++) {
			handlers[index] = switch (labelKey(labels[index])) {
				case "screen.serverutilities.text_style.reset" -> this::resetParts;
	...
	private StyledText.Effect effectForLabel(Component label) {
		...
		for (TextFormats.EffectOption option : TextFormats.EFFECTS.values()) {
			if (key.equals("screen.serverutilities.text_style.effect." + option.id())) {
	...
	private void attachAnvilTabs(int panelX, int panelY) {
		...
		anvilStyleTabButton = Button.builder(Component.translatable("screen.serverutilities.anvil.tab.style"), press -> {
				switchAnvilTab(AnvilEditorTab.STYLE);
				releaseButtonFocus(press);
			})
			.bounds(left, tabY, tabWidth, tabHeight)
			.build();
		anvilLoreTabButton = Button.builder(Component.translatable("screen.serverutilities.anvil.tab.lore"), press -> {
	...
	private void switchAnvilTab(AnvilEditorTab tab) {
		...
		if (tab == AnvilEditorTab.LORE) {
			...
			if (stack.isEmpty()) {
				showAnvilMessage(Component.translatable("screen.serverutilities.anvil.lore_no_item"));
				return;
			}
	...
	private void buildLoreWidgets(int panelX, int panelY, int lineCount) {
		...
		loreParagraphField = MultiLineEditBox.builder()
			.setX(fieldX)
			.setY(y)
			.setPlaceholder(Component.translatable("screen.serverutilities.anvil.lore_paragraph_hint"))
			.build(
				Minecraft.getInstance().font,
				fieldWidth,
				paragraphHeight,
				Component.translatable("screen.serverutilities.anvil.lore_paragraph")
			);
		...
		attach(iconButton(fieldX, y, fieldWidth, buttonH, Component.translatable("screen.serverutilities.anvil.lore_color_gradient"), new ItemStack(Items.MAGENTA_DYE), press -> {
		...
		attach(iconButton(fieldX, y, fieldWidth, buttonH, Component.translatable("screen.serverutilities.anvil.lore_wrap", loreWrapWidth), new ItemStack(Items.PAPER), press -> {
		...
		for (int index = 0; index < paragraphEffects.length; index++) {
			...
			int column = index % 2;
			int row = index / 2;
			Button button = Button.builder(Component.translatable("screen.serverutilities.text_style.effect." + effect.name().toLowerCase(java.util.Locale.ROOT)), press -> {
		...
		attach(iconButton(fieldX, y, fieldWidth, buttonH, Component.translatable("screen.serverutilities.anvil.lore_preview", generatedLoreLineCount()), new ItemStack(Items.WRITABLE_BOOK), press -> {
		...
		attach(iconButton(fieldX, actionRowY, fieldWidth, buttonH, Component.translatable("screen.serverutilities.anvil.lore_expand_style"), new ItemStack(Items.NAME_TAG), press -> openLoreLineStyle()));

		attach(iconButton(fieldX, actionRowY + buttonH + rowGap, fieldWidth, buttonH, Component.translatable("screen.serverutilities.anvil.lore_regenerate"), new ItemStack(Items.CRAFTING_TABLE), press -> {
			rebuildParagraphLoreFromDraft();
			applyLore();
		}));

		attach(iconButton(fieldX, footerY, half, buttonH, Component.translatable("screen.serverutilities.anvil.lore_reset"), new ItemStack(Items.BARRIER), press -> resetLore()));
	...
	private void buildLoreColorTable(int fieldX, int y, int fieldWidth, int buttonH, int rowGap) {
		...
		if (colorEditMode == ColorEditMode.GRADIENT) {
			...
			gradientStartButton = iconButton(fieldX, y, half, buttonH, Component.translatable("screen.serverutilities.text_style.gradient_start"), new ItemStack(Items.GLOW_INK_SAC), press -> {
				gradientTarget = GradientTarget.START;
				if (colorPicker != null) {
			...
			gradientEndButton = iconButton(fieldX + half + rowGap, y, half, buttonH, Component.translatable("screen.serverutilities.text_style.gradient_end"), new ItemStack(Items.INK_SAC), press -> {
		...
		attach(iconButton(fieldX, y, half, buttonH, Component.translatable("screen.serverutilities.text_style.apply"), new ItemStack(Items.EMERALD), press -> {
			applyLoreColorChoice(true);
	...
	private SelectionRange loreParagraphSelection() {
		...
		MultilineTextField textField = ((MultiLineEditBoxAccessor) loreParagraphField).serverutilities$getTextField();
		MultilineTextFieldAccessor accessor = (MultilineTextFieldAccessor) textField;
		int start = Math.min(accessor.serverutilities$getCursor(), accessor.serverutilities$getSelectCursor());
		int end = Math.max(accessor.serverutilities$getCursor(), accessor.serverutilities$getSelectCursor());
	...
	private void buildPartFields(int panelX, int panelY, int partCount, int contentTopOffset, TextStylePanelLayout.Metrics layout) {
		...
		if (layout.hasPartToggle(partCount)) {
			Component toggleLabel = partsCollapsed
				? Component.translatable("screen.serverutilities.text_style.parts_expand")
				: Component.translatable("screen.serverutilities.text_style.parts_collapse");
		...
		for (int index = 0; index < partCount; index++) {
			...
			EditBox field = new PartSelectEditBox(
				...
				Component.translatable("screen.serverutilities.text_style.part", partIndex + 1),
				() -> selectPart(partIndex)
			);
			field.setMaxLength(64);
			field.setValue(part.text);
			field.setHint(Component.translatable("screen.serverutilities.text_style.part", partIndex + 1));
		...
		if (partCount < MAX_PARTS) {
			attach(Button.builder(Component.translatable("screen.serverutilities.text_style.add_part"), press -> addPart())
	...
	private void attach(GuiEventListener widget, boolean interactive) {
		if (interactive) {
			if (widget instanceof Button button) {
				...
				screenInvoker.serverutilities$addRenderableWidget(editBox);
			} else if (widget instanceof ClassicColorSwatchButton swatchButton) {
				screenInvoker.serverutilities$addRenderableWidget(swatchButton);
			} else if (widget instanceof RgbColorPickerWidget picker) {
				screenInvoker.serverutilities$addRenderableWidget(picker);
			...
			screenInvoker.serverutilities$addRenderableOnly(renderable);
		}
		widgets.add(widget);
	...
	private enum LoreDynamicStyle {
		CLEAN("screen.serverutilities.anvil.lore_style.clean"),
		ACCENT("screen.serverutilities.anvil.lore_style.accent"),
		GRADIENT("screen.serverutilities.anvil.lore_style.gradient"),
		RARITY("screen.serverutilities.anvil.lore_style.rarity");
